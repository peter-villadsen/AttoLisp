...
public class StdLibTests
{
    ...
    public StdLibTests()
    {
        ...
        // Load stdlib.al from the repo (search a few candidate locations)
        var candidates = new List<string>
        {
            Path.Combine(AppContext.BaseDirectory, "..", "..", "..", "AttoLisp", "stdlib.al"),
            Path.Combine(AppContext.BaseDirectory, "..", "..", "..", "..", "AttoLisp", "stdlib.al"),
            Path.Combine(AppContext.BaseDirectory, "..", "..", "AttoLisp", "stdlib.al"),
            Path.Combine(Directory.GetCurrentDirectory(), "AttoLisp", "stdlib.al"),
        };
        // Walk up to 5 parents and check AttoLisp/stdlib.al
        var dir = new DirectoryInfo(AppContext.BaseDirectory);
        for (int i = 0; i < 5 && dir != null; i++)
        {
            var probe = Path.Combine(dir.FullName, "AttoLisp", "stdlib.al");
            candidates.Add(probe);
            dir = dir.Parent;
        }

        var stdlibPath = candidates.FirstOrDefault(File.Exists);
        if (stdlibPath == null)
        {
            throw new FileNotFoundException("stdlib.al not found for tests", string.Join(" | ", candidates));
        }
